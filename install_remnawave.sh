    ...
    while true; do
        ...
        while true; do
            ...
            if [[ "$key" == $'\x1b' ]]; then
                ...
                # Строки: пустая перед промптом + строка ввода + пустая + ошибка(2) + пустая + пустая + подсказка = 8 строк
                local lines_up=8
...
apply_template() {
    ...
    local base_url="https://raw.githubusercontent.com/DanteFuaran/Remna-install/main/templates/${template_id}"
...
main_menu() {
    ...
    alias dri="/usr/local/bin/dfc-remna-install" 2>/dev/null || true
